...
標準キーボードのprofileは`keyboard-ver1`です。prototypeは
`keyboard-ver0-prototype`、Waveshare 8.8inch touch panel kioskは
`touch-waveshare-8.8`を選びます。touch kioskの最短手順とDesktop autostartは
[INSTALL.mdのOption A2](INSTALL.md#option-a2-raspberry-pi-os-touch-panel-package)を参照してください。
...
Waveshare 8.8inch touch panel用profile packageを作る場合は、次のtargetへ置き換えます。
...
touch kioskではRaspberry Pi OS Desktop 64-bitを使い、DesktopとChromiumを残します。
...
touch panelではpackage名と適用profileを次のように置き換えます。coreとprofileのversionを
一致させ、同じapt transactionでinstallする原則は同じです。
