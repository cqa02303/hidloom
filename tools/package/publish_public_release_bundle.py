...
def build_plan(bundle: Path, repository: str, tag: str) -> dict[str, Any]:
    ...
    if not publication.get("ready"):
        blockers.extend(f"publication:{item}" for item in publication.get("blockers", []))
    smoke = manifest.get("hardware_smoke", {})
    if smoke.get("status") != "pass" or not isinstance(
        smoke.get("usable_keyboard_seconds"), (int, float)
    ) or smoke.get("usable_keyboard_seconds", 0) <= 0:
        blockers.append("hardware-smoke-not-passed")
    if manifest.get("packages", {}).get("touch_profile"):
        touch_smoke = manifest.get("touch_hardware_smoke") or {}
        if touch_smoke.get("status") != "pass" or not isinstance(
            touch_smoke.get("touch_ready_seconds"), (int, float)
        ) or touch_smoke.get("touch_ready_seconds", 0) <= 0:
    ...
    return {
        "schema": "hidloom.public-release-publish-plan.v1",
        "ready": not blockers,
        "blockers": blockers,
