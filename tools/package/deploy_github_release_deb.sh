...
Download and verify a GitHub Release split package set, then run the release
install flow on a Raspberry Pi.
...
if [ "$DRY_RUN" -eq 1 ]; then
    ...
    "$SCRIPT_DIR/install_github_release_deb.sh" --tag "$TAG" --repository "$REPOSITORY" --profile "$PROFILE" $REMOTE_ARG --dry-run --apt $keep_arg
    echo
