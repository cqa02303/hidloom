...
`build_touch_panel_release.sh`はtagやGitHub Releaseを作成しません。公開前は
`PACKAGE_RELEASE_MANIFEST.json`の`publication.ready`と実機smokeを別々にgateします。
...
portable `SHA256SUMS`を生成します。tag、GitHub upload、実機変更は行いません。公開前は
`--require-publication-ready --require-hardware-pass`を両方指定して検証します。
