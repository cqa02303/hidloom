...
download した `.sha256` が portable で `sha256sum -c` を通ることまで確認します。

公開後に GitHub Releases から別 directory へ asset を download し、portable sha256 と
`sha256sum -c` を確認する場合:
...
別環境で GitHub Release から `.deb` を取得して install する場合は、
...
実機へ copy して低レベルの `dpkg --dry-run -i`:
...
`release-deb-install` は package DB を変更し、dependency install は行わない低レベル入口です。通常は先に
