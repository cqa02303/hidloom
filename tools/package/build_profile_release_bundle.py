...
SCHEMA = "hidloom.profile-package-release.v2"
...
def write_notes(path: Path, manifest: dict[str, Any]) -> None:
    ...
    selected_status = channels["statuses"][selected]
    channel_status = (
        "ready"
    ...
    path.write_text(
        "\n".join(
            [
                ...
                f"- Release channel: `{selected}`",
                f"- Channel gate: `{channel_status}`",
                "- Internal RC binary assets must not be uploaded publicly before promotion to `stable-public`.",
                "",
                "## Download",
                "",
...
def build(args: argparse.Namespace) -> None:
    ...
    hardware_blockers = []
    if (
        hardware_smoke["status"] != "pass"
        or not hardware_smoke["device"]
        or not isinstance(hardware_smoke["touch_ready_seconds"], (int, float))
        or hardware_smoke["touch_ready_seconds"] <= 0
    ):
        hardware_blockers.append("hardware-smoke-not-passed")
...
def verify(
    ...
    provenance_path = directory / "PUBLIC_BUILD_PROVENANCE.json"
    expected_publication = publication_status(
        Path(__file__).resolve().parents[2],
        provenance_path if provenance_path.is_file() else None,
        str(manifest.get("profile", {}).get("id", "")),
        str(manifest.get("source", {}).get("commit", "")),
        str(manifest.get("packages", {}).get("version", "")),
    )
    if manifest.get("publication") != expected_publication:
        raise SystemExit("profile publication contract mismatch")
    smoke = manifest.get("hardware_smoke") or {}
    hardware_blockers = []
    if (
        smoke.get("status") != "pass"
        or not smoke.get("device")
        or not isinstance(smoke.get("touch_ready_seconds"), (int, float))
        or smoke.get("touch_ready_seconds", 0) <= 0
    ):
        hardware_blockers.append("hardware-smoke-not-passed")
    ...
    if manifest.get("release_channels") != expected_channels:
        raise SystemExit("profile release channel contract mismatch")
...
def main() -> None:
    ...
    build_parser.add_argument(
        "--channel", choices=BINARY_RELEASE_CHANNELS, default=INTERNAL_RC_CHANNEL
    )
    build_parser.add_argument(
    ...
    verify_parser.add_argument("--require-channel-ready", choices=BINARY_RELEASE_CHANNELS)
