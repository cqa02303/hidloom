...
def assign_public_identity(export: Path) -> None:
    identity_path = export / "config/public-usb-identity.json"
    identity = json.loads(identity_path.read_text(encoding="utf-8"))
    identity["assignment"]["status"] = "assigned"
    identity["assignment"]["allocation_evidence"] = {
        "merged_date": "2026-07-21",
        "merged_at": "2026-07-21T12:34:56Z",
        ...
        "pull_request_url": "https://github.com/pidcodes/pidcodes.github.com/pull/1246",
    ...
    identity["profiles"]["public_formal"]["status"] = "assigned-ready"
    identity["profiles"]["public_formal"]["public_release_allowed"] = True
    write_export_json(export, "config/public-usb-identity.json", identity)
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        release_internal_ready = workspace / "release-internal-ready"
        ...
        assert manifest["schema"] == "hidloom.public-release-bundle.v5"
        ...
        assert manifest["release_channels"]["selected"] == "internal-rc"
        assert manifest["release_channels"]["selected_ready"] is False
        assert manifest["release_channels"]["statuses"]["source-public"]["ready"] is True
        assert manifest["release_channels"]["statuses"]["internal-rc"]["blockers"] == [
            "public-build-provenance-missing",
            "hardware-smoke-not-passed",
        ]
        assert manifest["release_channels"]["statuses"]["stable-public"]["blockers"] == [
            "public-usb-identity-not-assigned",
            "public-build-provenance-missing",
            "hardware-smoke-not-passed",
        ]
        ...
        internal_failed = subprocess.run(
            [
                "python3",
                str(export / "tools/public_release_bundle.py"),
                "--verify",
        ...
        assert internal_failed.returncode != 0
        assert "public-build-provenance-missing" in internal_failed.stderr
        ...
        internal_ready_command = list(provenance_command)
        internal_ready_command[
            internal_ready_command.index(str(release_with_provenance))
        ] = str(release_internal_ready)
        internal_ready_command.extend(
            ["--hardware-smoke-status", "pass", "--usable-keyboard-seconds", "6.0"]
        )
        internal_ready_created = subprocess.run(
            internal_ready_command,
            cwd=export,
            capture_output=True,
            text=True,
        )
        assert internal_ready_created.returncode == 0, (
            internal_ready_created.stdout + internal_ready_created.stderr
        )
        internal_ready_manifest = json.loads(
            (release_internal_ready / "RELEASE_MANIFEST.json").read_text(encoding="utf-8")
        )
        assert internal_ready_manifest["release_channels"]["selected"] == "internal-rc"
        assert internal_ready_manifest["release_channels"]["selected_ready"] is True
        assert internal_ready_manifest["release_channels"]["statuses"]["internal-rc"][
            "blockers"
        ] == []
        assert internal_ready_manifest["release_channels"]["statuses"]["stable-public"][
            "blockers"
        ] == ["public-usb-identity-not-assigned"]
        subprocess.run(
            [
        ...
        assert touch_manifest["release_channels"]["statuses"]["internal-rc"]["blockers"] == [
            "touch-hardware-smoke-not-passed"
        ]
        ...
        assert plan["release_channel"] == "internal-rc"
        assert "release-channel-not-stable-public" in plan["blockers"]
        assert "release-channel:public-usb-identity-not-assigned" in plan["blockers"]
        assert "release-channel:hardware-smoke-not-passed" not in plan["blockers"]
        assert "release-channel:touch-hardware-smoke-not-passed" in plan["blockers"]
        ...
        original_identity = json.loads(
            (export / "config/public-usb-identity.json").read_text(encoding="utf-8")
        )
        assign_public_identity(export)
        publishable_command = list(touch_command)
        publishable_command[publishable_command.index(str(release_with_touch))] = str(
            publishable
        )
        publishable_command.extend(
            [
                "--touch-hardware-smoke-status",
                "pass",
                "--touch-ready-seconds",
        ...
        publishable_created = subprocess.run(
            publishable_command,
            cwd=export,
            capture_output=True,
            text=True,
