...
def run(
    ...
    *args: str,
    ...
    gh_wrapper = environment.get("HIDLOOM_TEST_GH_WRAPPER")
    if gh_wrapper:
        command.extend(["--gh-command", sys.executable, gh_wrapper])
    command.extend(
        ["--root", str(root), "--upstream-checkout", str(upstream), *args]
    )
...
def main() -> None:
    with tempfile.TemporaryDirectory() as temporary:
        ...
        gh_wrapper = temporary_path / "gh-wrapper.py"
        gh_wrapper.write_text(
            "import os\n"
            "import sys\n"
            "if sys.argv[1:3] != ['pr', 'view']:\n"
            "    raise SystemExit(64)\n"
            "print(os.environ['HIDLOOM_TEST_PR_JSON'])\n",
            encoding="utf-8",
            newline="\n",
        )
        environment = {
            "HIDLOOM_TEST_GIT_WRAPPER": str(git_wrapper),
            "HIDLOOM_TEST_GH_WRAPPER": str(gh_wrapper),
