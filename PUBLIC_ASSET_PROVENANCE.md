# HIDloom Public Asset Provenance

- Ready: `true`
- Asset records: 9
- Assets: 51
- Missing declarations: 0
- Stale declarations: 0

| Record | Category | Provenance | License | Assets | Review |
|---|---|---|---|---:|---|
| keyboard-layout-images | hardware-documentation | project-authored | GPL-3.0-or-later | 2 | complete |
| keyboard-case-cad | mechanical-design | project-authored | GPL-3.0-or-later | 2 | complete |
| hidloom-web-icons | web-branding | deterministically-generated | GPL-3.0-or-later | 6 | complete |
| hidloom-webui-gallery | software-documentation | project-authored | GPL-3.0-or-later | 14 | complete |
| keyboard-hardware-photo-gallery | hardware-documentation | project-authored | GPL-3.0-or-later | 8 | complete |
| touch-panel-photo-gallery | hardware-documentation | project-authored | GPL-3.0-or-later | 2 | complete |
| system-overview-diagram | architecture-documentation | project-authored | GPL-3.0-or-later | 1 | complete |
| keyboard-kicad-designs | electrical-design | project-authored | GPL-3.0-or-later | 14 | complete |
| keyboard-hardware-pdf-exports | electrical-design-reference | project-generated | GPL-3.0-or-later | 2 | complete |

## Findings

- None
