...
- `logicd` からの layer / mode / daemon status / alert / script exit 通知の表示
...
USB HID transport (`hidd` / legacy `usbd`) が active の時は、debug / `uinput` runtime mode 中でも接続 status として USB を表示します。
