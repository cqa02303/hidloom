...
`Web UI QR`を有効にすると、現在のroutable IPv4 addressを`HTTPS://<address>`としてQR Code Version 1-Lへ格納します。
`KC_SH3`からは同じ生成処理を使う15秒の一時QR表示を呼び出せます。Ready画面の固定表示設定は変更しません。
21×21 modulesを2倍で描き、四辺4 modulesのquiet zoneを含む58×58pxとして64px幅の中央に配置します。
QRにはBasic認証password、cookie、CSRF tokenを含めません。routable IPv4を解決できない場合やVersion 1の25文字を超える場合は
`Web UI / No address`を表示します。既定はOFFで、HTTP OLED editorから表示と順序を選びます。
表示仕様、互換性、検証、商標注記は[OLED Web UI QR](../../docs/feature/oled-web-ui-qr.md)を参照してください。
