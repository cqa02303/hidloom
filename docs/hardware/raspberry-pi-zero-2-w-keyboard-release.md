...
`RELEASE_MANIFEST.json`の`release_channels.selected=stable-public`かつ`selected_ready=true`でなく、
keyboardの`hardware_smoke`が`pass`でない候補は公開配布用ではありません。touch profileを含むReleaseでは
