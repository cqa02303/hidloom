...
- 2026-07-16の後続監査で、generic `console` gettyとRaspberry Pi post-buildが追加する`tty1`
  gettyが、`console=tty1`上で同じHDMI terminalを取り合うことを特定した。M6は
  `BR2_TARGET_GENERIC_GETTY_PORT="tty1"`に固定し、artifact verifierでactive gettyが`tty1` 1個だけか
  確認する。nativeとARM QEMUの両方で`USB -> uinput -> pi/Enter -> USB`往復もbuild gateにする。
- single-getty source `f4a5690b06f0`のclean public exportから生成したraw imageのSHA-256は
  `5ec1342a0d5d6e8705419998f4298a8782fe2dcf713955f864fe053c14ea17ff`。最終rootfsのactive getty 1行、
  credential/sudo、ARM import、split route、native/ARM uinput往復、provenance/readinessをpassした。
