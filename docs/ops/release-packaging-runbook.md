...
この時点ではtag作成もuploadも行いません。公開候補ではさらに次の二つを通します。

```bash
python3 tools/package/build_profile_release_bundle.py verify \
  build/touch-panel-release --require-hardware-pass
python3 tools/package/build_profile_release_bundle.py verify \
  build/touch-panel-release --require-publication-ready
```

前者はRaspberry Pi 4実機のdevice名とtouch-ready時間を要求し、後者はpublic build provenanceと
正式割当済みUSB identityを要求します。`RELEASE_NOTES.md`をGitHub Release本文、directory内の
各fileをRelease assetとして使います。
...
`publication.ready=true`、keyboardとtouchのhardware smoke `pass`、final public source build、
人間によるplan確認の後にだけ
...
helperは既存Releaseと既存tagを拒否し、`gh release create --draft --prerelease --target <source-commit>`へ
