...
| `tools/package/deploy_deb_verify.sh` | package layout、manifest、systemd state、smoke を確認する | `--smoke` の時だけ live smoke を実行 |
...
`deb-verify-XX` は package version、manifest、systemd FragmentPath、
...
4. install 後に `shadowed-by-etc` が出るか。
5. unit switch dry-run で `will-backup-remove`、`already-package-unit`、`missing-package-unit` のどれが出るか。
6. switch 後に FragmentPath が package unit 側を指すか。
7. `deb-verify-smoke-XX` で live smoke が通るか。
